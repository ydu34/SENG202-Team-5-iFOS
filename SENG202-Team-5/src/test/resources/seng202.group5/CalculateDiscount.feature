Feature: Calculate Discount
  ...
  Scenario: Worker confirms an order for a customer which is registered.
    ...
    When Order gets payed $10.00
    Then Customer now has 6 purchase points

  Scenario: Worker confirms an order for a customer which is registered.
    Given Order Costs $155.00
    And Customer has 0 purchase points
    When Order gets payed $155.00
    Then Customer now has 15 purchase points

  Scenario: Worker confirms an order for a customer which is registered.
    Given Order Costs $10.00
    And Customer has 5 purchase points
    When Order gets payed $9.00
    Then Customer now has 5 purchase points